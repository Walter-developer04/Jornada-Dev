...
// Exemplo 1: Definindo um objeto simples com type
type ProdutoComType = {
  nome: string;
  preco: number;
};

// Exemplo 2: Definindo o mesmo objeto com interface
interface ProdutoComInterface {
  nome: string;
  preco: number;
}

// Ambos podem ser usados da mesma forma para tipar objetos
const notebookType: ProdutoComType = { nome: "Notebook", preco: 3000 };
const notebookInterface: ProdutoComInterface = { nome: "Notebook", preco: 3000 };

console.log(notebookType);
console.log(notebookInterface);

// Exemplo intermediário: type permite uniões, interface não diretamente
...
const pedido: Pedido = { id: 1, status: "pendente" };
console.log(pedido);

// Exemplo prático: type para funções e interseções
type FuncaoCallback = (mensagem: string) => void;
...
type UsuarioComAuditoria = ProdutoComType & RegistroAuditoria;

const usuarioComAuditoria: UsuarioComAuditoria = {
  nome: "Maria",
  preco: 99.90,
...
// Exemplo com interface: extensão e implementação
interface Entidade {
  id: number;
}
...
const cliente: Cliente = { id: 1, nome: "João", email: "joao@email.com" };
console.log(cliente);
