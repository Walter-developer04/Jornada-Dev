...
// Exemplo simples: tipo híbrido que é função e tem propriedade
interface SaudacaoHibrida {
  (nome: string): string;   // pode ser chamada como função
  idioma: string;           // propriedade adicional
}

// Implementação do tipo híbrido
const saudacao: SaudacaoHibrida = (nome: string) => `Olá, ${nome}!`;
saudacao.idioma = "Português";

console.log(saudacao("Maria")); // Uso como função
console.log(saudacao.idioma);   // Acesso à propriedade

// Exemplo intermediário: função com estado interno e métodos
interface ContadorHibrido {
  (): void;                  // chamada para incrementar
  valor: number;             // estado
  reset(): void;             // método
}

function criarContador(): ContadorHibrido {
  const contador = (() => {
    contador.valor++;
  }) as ContadorHibrido;
...
// Exemplo prático: função que valida dados e possui metadados
interface ValidadorHibrido {
  (dado: any): boolean;      // validação
  mensagemErro: string;      // mensagem associada
  atualizarMensagem(novaMensagem: string): void;
}

function criarValidadorEmail(): ValidadorHibrido {
  ...
  validador.atualizarMensagem = (novaMensagem: string) => {
